...
function preload() {
  ...
  // flaskImg = loadImage('FlaskB.png', loaded);
...
function setup() {
  ...
  aftercolour = color(231, 232, 232); // Defining the colour after titration
...
if (showppt==true){
  ...
  image(bureteImg, width / 2 - 6.05 * size-3, .7 * size, 9.88 * size / 1, buretesize * 2.3 * size / 1);
  noStroke();
  pop();
  if (flaskTouched) {
    noStroke();
